...
19. **`ExpandingPanel` (the default variant — used by `AccordianCore`'s default, `ContentDocs`'s docs/admin nav, and directly by consumers) drove its entire collapse animation and `content-is-on-top` overlay positioning through the `::details-content` pseudo-element, which WebKit (Safari/iOS) does not support as of 2026-08.** WebKit silently drops the whole rule block — no console warning, no fallback — so on any real Apple device: no expand/collapse animation, and critically, `content-is-on-top`'s `position: absolute` never applies, meaning an overlay panel (e.g. a mobile nav dropdown, or any "float over trailing content" panel) renders in normal document flow instead of overlaying, breaking the layout around it. Confirmed via real-device testing (not reproducible in Chrome DevTools' device emulator, since that's still the Blink engine — only real WebKit exhibits this). `ExpandingPanelClassic` was already immune, since it uses the older, universally-supported `grid-template-rows: 0fr → 1fr` technique on a real class instead of a pseudo-element — but nothing routed non-supporting browsers to it; a consumer had to explicitly opt into `variant="classic"`. Fixed 2026-08-22: `ExpandingPanel` now carries an `@supports not selector(::details-content)` fallback block that reproduces Classic's exact technique (including wrapping the content slot in a new `.inner` div — the `overflow: hidden` needs to live on the slotted content's own wrapper, not just the outer grid container, or a plain child's default `min-height: auto` stops the row collapsing to true `0`; template change, snapshots updated across every consumer's test suite, all passing). When relying on a very new CSS feature (an experimental pseudo-element, `@starting-style`, `appearance: base-select`, etc.) for anything load-bearing rather than purely decorative, check WebKit support specifically — Chrome's device-emulation mode cannot catch a WebKit-only gap, only a real Safari/iOS session (or Safari's remote Web Inspector against a real device) can.
