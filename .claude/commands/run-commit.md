...
3. Draft the commit message following the repo's existing style (`git log --oneline -10`), and create the commit with a HEREDOC so formatting is preserved, ending with the `Co-Authored-By:` trailer for the model currently running this session (use the exact attribution line from the session's system instructions; never hardcode a model name here).
